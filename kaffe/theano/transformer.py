...
class TheanoNode(object):
    '''An intermediate representation for Theano operations.'''

    def __init__(self, op, *args, **kwargs):
        # A string corresponding to the Theano operation
        self.op = op
...
class MaybeActivated(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        return TheanoNode(*args, **kwargs)


class TheanoMapper(NodeMapper):
    ...
    def map_pooling(self, node):
        ...
        return TheanoNode(pool_op, kernel_params.kernel_h, kernel_params.kernel_w,
    ...
    def map_lrn(self, node):
        ...
        return TheanoNode('lrn', int(params.local_size / 2), alpha, params.beta)

    def map_concat(self, node):
        axis = (2, 3, 1, 0)[node.parameters.axis]
        return TheanoNode('concat', axis)

    def map_dropout(self, node):
        return TheanoNode('dropout', node.parameters.dropout_ratio)
    ...
    def map_eltwise(self, node):
        ...
        try:
            return TheanoNode(operations[op_code])
        except KeyError:
...
class TheanoEmitter(object):
    ...
    def emit_imports(self):
        return self.statement('from kaffe.theano import Network\n')
...
class TheanoTransformer(object):
    ...
    def transform_data(self):
        if self.params is None:
            transformers = [
                ...
                # Reshape the parameters to Theano's ordering
                DataReshaper({
                    # (c_o, c_i, h, w) -> (h, w, c_i, c_o)
                    #NodeKind.Convolution: (2, 3, 1, 0),
                    # Theano follows the same layout of convolution params
                    # as the Caffe
    ...
    def transform_source(self):
        if self.source is None:
            mapper = TheanoMapper(self.graph)
            chains = mapper.map()
            emitter = TheanoEmitter()
