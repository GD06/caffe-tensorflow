...
class Network(object):
    ...
    def conv(self,
        ...
        convolve = lambda i, k: tf.nn.conv2d(i, k, [1, 1, s_h, s_w],
                                             padding=padding,
                                             data_format='NCHW')
        with tf.variable_scope(name) as scope:
            ...
            if group == 1:
                ...
                input_groups = tf.split(axis=1, num_or_size_splits=group,
                                        value=input)
                kernel_groups = tf.split(axis=3, num_or_size_splits=group,
                                         value=kernel)
    ...
    def lrn(self, input, radius, alpha, beta, name, bias=1.0):
        ...
        lrn = tf.nn.local_response_normalization(input,
                                                  ...
                                                  bias=bias,
                                                  name=name)
        return tf.transpose(lrn, perm=[0, 3, 1, 2])
    ...
    def batch_normalization(self, input, name, scale_offset=True, relu=False):
        ...
        with tf.variable_scope(name) as scope:
            ...
            if self.trainable:
                ...
                output, batch_mean, batch_var = tf.nn.fused_batch_norm(
                    input,
                    scale=scale,
                    offset=offset,
